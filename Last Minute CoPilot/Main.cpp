	...
	while(!done)
	{
		...
		if(ev.type == ALLEGRO_EVENT_TIMER)
		{
			...
			if(keys[UP]) {
			    ...
			    my3DWorld->moveCameraUp();
                }
			else if(keys[DOWN]) {
			    World_Input.cameraDOWN();
			    my3DWorld->moveCameraDown();
                }

			if(keys[LEFT]) {
                World_Input.cameraLEFT();
                my3DWorld->moveCameraLeft();
                }
			else if(keys[RIGHT]) {
			    World_Input.cameraRIGHT();
			    my3DWorld->moveCameraRight();
                }
            if(keys[KEY_Q]){
            ...
            if(keys[KEY_W]){
                my3DWorld->moveCameraForward();
                gameInput.Axis_Y = -1;
            }
            if(keys[KEY_A]){
                gameInput.Axis_X = -1;
                my3DWorld->rotateCameraLeft();
            }
            if(keys[KEY_S]){
                gameInput.Axis_Y = 1;
                my3DWorld->moveCameraBackward();
            }
            if(keys[KEY_D]){
                gameInput.Axis_X = 1;
                my3DWorld->rotateCameraRight();
            }
